...
#define STRING_CONFIG_H_AUTHOR "(3D.uy, versión en Español)" // Quién hizo los cambios.
...
/**
 * Selecciona un puerto serial para comunicarte con el protocolo RS485.
 * :[-1, 0, 1, 2, 3, 4, 5, 6, 7]
 */
...
// Diámetro de filamento generalmente esperado (1.75, 2.85, 3.0, ...). Utilizado para Volumétrico, Sensor de Ancho de Filamento, etc.
// El extrusor original de Anet A8 está diseñado para 1.75mm
...
// Valores de resistencia al utilizar sensores MAX31865 (-5) en TEMP_SENSOR_0 / 1
  ...
  #define TEMP_RESIDENCY_TIME          6  // (segundos) Tiempo de espera para que el hotend se "estabilice" en M109
...
#if TEMP_SENSOR_BED
  #define TEMP_BED_RESIDENCY_TIME      6  // (segundos) Tiempo de espera para que la cama se "estabilice" en M190
...
#if ENABLED(PIDTEMP)
  //#define PID_DEBUG             // Imprimir datos de depuración de PID en el puerto serie. Use 'M303 D' para alternar la activación.
  //#define PID_PARAMS_PER_HOTEND // Utilizar parámetros de PID separados para cada extrusor (útil para extrusores desparejados)
                                  // Establecer/obtener con código G: M301 E[número de extrusor, 0-2]
  
  // ANET A8 Extrusor Estándar a 210 grados Celsius y 100% de Ventilador
  // (medido después de M106 S255 con M303 E0 S210 C8)
  #if ENABLED(PID_PARAMS_PER_HOTEND)
    // Especifique hasta un valor por cada extrusor aquí, según su configuración.
    // Si hay menos valores, el último se aplica a los extrusores restantes.
    //
...
#define PIDTEMPBED
  ...
  //#define PID_BED_DEBUG // Imprimir datos de depuración del PID de la cama en el puerto serie.

  // ANET A8
  // Cama original + 0.3 mm de conducción de calor en vidrio borosilicato de 4 mm (PID-Autotune: M303 E-1 S60 C5):
  #define DEFAULT_bedKp 22.20
  #define DEFAULT_bedKi 1.08
  #define DEFAULT_bedKd 114.00
  // ENCUENTRA LOS TUYOS: "M303 E-1 C8 S90" para ejecutar la sintonización automática en la cama a 90 gradosC durante 8 ciclos.
#else
  //#define BED_LIMIT_SWITCHING   // Mantener la temperatura de la cama dentro de BED_HYSTERESIS del objetivo
#endif

// Agrega 'M190 R T' para un enfriamiento gradual adicional de la cama con 'M190 R'.
 ...
 * Si esta opción está deshabilitada, se utilizará el método bang-bang y CHAMBER_LIMIT_SWITCHING 
 * habilitará la histéresis.
 *
 * La frecuencia del PID será la misma que la del PWM del extrusor.
 * Si PID_dT es el valor predeterminado y correcto para el hardware/configuración, esto significa 7,689 Hz,
 * lo cual es adecuado para generar una onda cuadrada en una carga resistiva y no afecta significativamente
...
#define MAX_CHAMBER_POWER 255 // limita el ciclo de trabajo del calentador de la cámara; 255 = corriente máxima
  ...
  #define PID_FUNCTIONAL_RANGE 15 // Si la diferencia de temperatura entre la temperatura objetivo y la temperatura actual
...
//#define MARKFORGED_YX
  ...
  //#define MARKFORGED_INVERSE  // Habilitar para una cinemática Markforged con trayectoria de correa invertida
#endif

// Habilitar para una impresora con estilo de correa con movimiento "Z" infinito
//#define BELTPRINTER
 ...
 * Funciona para movimientos lentos, pero no funciona bien con movimientos rápidos. 
 * Se debe implementar una compensación de extrusión más complicada.
 *
 ...
 * es más probable que queme más material de lo que debería. Por lo tanto, se implementaría una 
 * compensación similar para las operaciones de corte láser.
 *
 * Fresado:
 ...
 */
#define DEFAULT_ACCELERATION                   400   // Aceleración X, Y, Z... y E para movimientos de impresión
#define DEFAULT_RETRACT_ACCELERATION          1000   // Aceleración E para retracciones
#define DEFAULT_TRAVEL_ACCELERATION           1000   // Aceleración X, Y, Z... para movimientos de desplazamiento (no impresión)
 ...
 */
  ...
  #define JUNCTION_DEVIATION_MM 0.010 // (mm) Distancia desde el borde real de la unión
...
//#define BD_SENSOR
  ...
  //#define BD_SENSOR_PROBE_NO_STOP // Sonda la cama sin detenerse en cada punto de sonda
#endif
...
// Para M851, proporcionar rangos para ajustar los desplazamientos de la sonda en X, Y y Z
...
// Habilitar la prueba de repetibilidad M48 para evaluar la precisión de la sonda
...
//#define Z_CLEARANCE_FOR_HOMING  4   // (mm) Altura mínima de Z antes de la homing (G28) para el espacio libre de Z sobre la cama, abrazaderas, ...
                                      // Necesitarás este espacio libre por encima de Z_MAX_POS para evitar rozamientos.

//#define Z_AFTER_HOMING         10   // (mm) Altura a la que moverse después de la homing (si Z se homologó)
//#define XY_AFTER_HOMING { 10, 10 }  // (mm) Mover a una posición XY después de la homing (y elevando Z)

//#define EVENT_GCODE_AFTER_HOMING "M300 P440 S200"  // Comandos para ejecutar después de G28 (y moverse a XY_AFTER_HOMING)
 ...
 * homing, mientras que el otro se puede utilizar para cosas como SD_ABORT_ON_ENDSTOP_HIT.
 */
...
/**
 * Comandos para ejecutar al final del probing G29.
...
//#define DEBUG_LEVELING_FEATURE
  ...
  /**
   * Agregar compensación Z (M424 Z) que se aplica a todos los movimientos a nivel de planificación.
   * Esta compensación Z se establecerá automáticamente en el valor medio con G29.
   */
...
//
// LCD SR sin bloqueo de 2 cables de https://github.com/fmalpartida/New-LiquidCrystal/wiki/schematics#user-content-ShiftRegister_connection
 ...
 *
 * CREALITY_TOUCH
 *  - CR-6 OEM pantalla táctil. Una pantalla DWIN con capacidad táctil.
 *
...
//#define TFT_LVGL_UI
 ...
 /**
 * Tema TFT para Color_UI. Elija uno de los siguientes o agregue uno nuevo al directorio 'Marlin/src/lcd/tft/themes'
 *
 * BLUE_MARLIN  - Tema predeterminado con fondo de color 'azul medianoche'
 * BLACK_MARLIN - Tema con fondo de color 'negro'
 * ANET_BLACK   - Tema utilizado para Anet ET4/5
 */
#define TFT_THEME BLACK_MARLIN

//#define TFT_SHARED_IO   // La entrada/salida se comparte entre la pantalla TFT y otros dispositivos. Desactivar la transferencia de datos asíncrona.

#define COMPACT_MARLIN_BOOT_LOGO  // Utilizar datos comprimidos para ahorrar espacio en la memoria Flash
#endif
